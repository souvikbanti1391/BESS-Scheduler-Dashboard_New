...
# ============================================================
#   IEX MCP Predictor — Full Visual Analytics Suite (ready)
# ============================================================

# PATH fix so utils imports work in Streamlit Cloud
import sys
...
try:
    ...
    with col1:
        st.metric("Highest MCP (Rs/kWh)", f"{insights['highest'][0]:.3f}", f"@ {insights['highest'][1]}")
        st.metric("Lowest MCP (Rs/kWh)", f"{insights['lowest'][0]:.3f}", f"@ {insights['lowest'][1]}")
    with col2:
            ...
            try:
                with st.spinner("Running backend forecast..."):
                    ...
                    if not forecast_df.empty:
                        ...
                        st.plotly_chart(gradient_line_with_delta(forecast_df), use_container_width=True)
                    else:
